...
Reward function $J(\theta)$ 는 다음과 같이 정의할 수 있음:
...
그러면 으레 하듯이 이 reward function 의 gradient 를 계산하여 maximize 하면 되는 건데, 여기서 문제는 state distribution $$d^\pi(s)$$ 의 gradient 를 계산하기가 어렵다는 것이다.
